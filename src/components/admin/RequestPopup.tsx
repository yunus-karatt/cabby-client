...
const RequestPopup = ({ driver, setPopup, setRequest }: RequestPopupProps) => {
  ...
  const verifyDriver = async () => {
    try {
      ...
      setRequest((prev) => {
        return prev?.filter((req) => req._id !== driver._id);
      });
      setPopup(false);
      } catch (error) {
        console.log(error)
    ...
    try {
      Swal.fire({
        ...
        preConfirm: async (reason) => {
          try {
            await adminAxios.post(adminApi.rejectDriver, {
              id: driver._id,
              reason,
            });
          } catch (error) {
  ...
  return (
    <>
      <div
        className={`bg-white min-w-[460px] border absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 rounded-md overflow-auto no-scrollbar ${
          src &&
          "flex flex-col gap-y-3 justify-center items-center w-auto"
        }`}
      >
        {src ? (
          <>
          ...
          <div >
            ...
            <div className="flex flex-col p-3 ms-10 gap-y-2 min-w-[82%]">
              ...
              <div className="flex mt-3 justify-start flex-col md:flex-row items-center gap-x-3">
                <div className=" object-contain flex flex-col gap-y-2 ">
                  {loading && (
                  ...
                  <img
                    onClick={() =>
                      setSrc({
                        src: driver.vehicleDocuments.registration
                          .registrationImage,
                        text: "RC",
                      })
                    }
                    className="cursor-pointer h-[160px] w-[160px]"
            ...
            <div className="flex justify-center gap-x-5 mt-8">
              <button
                ...
                className="w-[180px] py-2 px-6 rounded-lg border border-primary text-primary"
              >
                Decline
              </button>
              <button
                onClick={verifyDriver}
                className="w-[180px] py-2 px-6 rounded-lg bg-primary text-white"
              >
                Accept
              </button>
            </div>
