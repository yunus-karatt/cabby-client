...
const VehicleDetails = ({
  ...
  const [errors, setErrors] = useState<{ file: string; inputNumber: string }>({
    file: "",
    inputNumber: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    if (!inputNumber.trim()) {
      setErrors((prev) => ({ ...prev, inputNumber: `${role} number is required.` }));
      return;
    }
    try {
  ...
  return (
    <div className="w-[480px] h-[570px] bg-secondary overflow-y-auto">
      <CabbyNav />
      ...
      <p className="text-danger m-5 text-center">{errors.file}</p>
      <div className="flex flex-col p-3 ms-10 gap-y-2 w-[82%] my-5">
        ...
        />
        <p className="text-danger">{errors.inputNumber}</p>
      </div>
